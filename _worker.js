// ================================================================
// KWEN COUNCIL — Cloudflare Pages Worker
// File: _worker.js (Service Worker format for Pages)
// ================================================================

const SAFE_AI_PREAMBLE = `MANDATORY SAFETY GUIDELINES:
...
3. On medical/legal/financial topics, frame responses as analysis tools not advice.
...
6. No manipulation, rhetorical tricks, or persuasion bypassing rational thought.
...
You are a council assembler for a critical thinking research tool. Select 3-5 specialist personas whose perspectives will create the MOST INTELLECTUALLY PRODUCTIVE FRICTION on this topic — genuine disagreement on foundational assumptions, not just different angles.

TOPIC: "${topic}"

RULES:
1. Prioritize perspectives that disagree on FUNDAMENTAL ASSUMPTIONS
2. Include at least one heterodox/contrarian voice challenging mainstream consensus
...
- core_commitment: ONE non-negotiable belief creating friction with others
- friction_with: array of 2-3 reasoning types they always challenge
...
- known_bias: their honest intellectual blind spot
...
ANTI-HOMOGENIZATION RULES — violating these defeats your purpose:
1. Push back against: ${Array.isArray(persona.friction_with) ? persona.friction_with.join(', ') : persona.friction_with}
2. NEVER use these phrases: ${Array.isArray(persona.forbidden_rhetoric) ? persona.forbidden_rhetoric.join(' | ') : persona.forbidden_rhetoric}
3. Write ONLY in ${persona.vocabulary_register} register
4. Do NOT open by validating other perspectives
5. Find at least one point of friction with the question's framing
6. Argue as ${persona.name} — your vocabulary, habits of thought, blind spots included

Other council members (you respond independently): ${others}

RESPONSE: 150-220 words. Lead with your strongest insight. No padding.`;
}
...
You are a synthesis engine. Synthesize WITHOUT papering over genuine disagreements.
...
{
  "summary": "3-4 paragraphs identifying genuine disagreements (name which members hold which positions), unexpected convergences, and preserved tensions — do NOT force consensus",
  "decision_framework": {
    "key_questions": ["3-5 questions a critical thinker should ask, derived from the council tensions"],
    "evidence_that_would_change_views": ["for each perspective, what evidence would shift it"],
    "red_flags": ["3-4 warning signs the council collectively identified"]
  },
  "open_questions": [
    { "question": "genuinely unresolved question", "why_unresolved": "specific reason — methodological, empirical, definitional, or political" }
  ]
}`;
}

// ================================================================
// DEFAULT COUNCIL FALLBACK
// ================================================================
  ...
  return [
    {
      ...
      epistemology: "Knowledge is validated only through reproducible evidence and falsifiable hypotheses.",
      ...
      known_bias: "Systematically undervalues qualitative and experiential knowledge.",
      signature_approach: "Leads with what evidence shows, then states explicitly what it cannot tell us.",
      avatar_initials: "MW"
    },
    {
      ...
      epistemology: "Understanding requires examining power structures and assumptions embedded in the question itself.",
      core_commitment: "Every framework contains hidden normative commitments that must be surfaced.",
      friction_with: ["naive empiricism ignoring social construction", "instrumental reasoning", "false value-neutrality"],
      forbidden_rhetoric: ["the data shows", "practically speaking", "at the end of the day", "common sense"],
      vocabulary_register: "philosophical",
      known_bias: "Can over-complicate questions that have clear practical answers.",
      signature_approach: "Historicises and deconstructs the question before attempting to answer it.",
      avatar_initials: "AC"
    },
    {
      ...
      friction_with: ["purely theoretical approaches", "solutions ignoring incentive structures", "ahistorical analysis"],
      ...
      known_bias: "Can be overly accommodating of existing power structures.",
...
// HF API
// ================================================================
async function callQwen(messages, hfToken, model, opts) {
  opts = opts || {};
  ...
  if (!resp.ok) {
    ...
    throw new Error('HF API ' + resp.status + ': ' + t.slice(0, 200));
  }
  const data = await resp.json();
  if (!data.choices || !data.choices[0]) throw new Error('Unexpected HF response');
  return data.choices[0].message.content;
}

async function buildCouncil(topic, hfToken, model) {
  try {
    const text = await callQwen([{ role: 'user', content: councilPrompt(topic) }], hfToken, model, { max_tokens: 2500, temperature: 0.88 });
...
async function personaReply(persona, allPersonas, question, history, hfToken, model) {
  const idx = allPersonas.findIndex(p => p.id === persona.id);
  const hist = (history || []).slice(-3).flatMap(h => {
    const pr = h.responses && h.responses[idx];
    if (!pr) return [{ role: 'user', content: h.question }];
    return [{ role: 'user', content: h.question }, { role: 'assistant', content: pr }];
  });
  return callQwen(
    [{ role: 'system', content: personaPrompt(persona, allPersonas) }, ...hist, { role: 'user', content: question }],
    hfToken, model, { max_tokens: 450, temperature: 0.74 }
  );
}

async function buildSynthesis(question, council, responses, hfToken, model) {
  try {
    const text = await callQwen([{ role: 'user', content: synthesisPrompt(question, council, responses) }], hfToken, model, { max_tokens: 1800, temperature: 0.62 });
    const clean = text.replace(/```json\s?/g, '').replace(/```\s?/g, '').trim();
...
async function getSession(request, env) {
  const sid = cookieSession(request);
  if (!sid) return null;
  try {
...
// HANDLERS
// ================================================================
async function onLogin(request, env) {
...
async function onCallback(request, env) {
  ...
  const ur = await fetch('https://huggingface.co/oauth/userinfo', { headers: { 'Authorization': 'Bearer ' + td.access_token } });
  const ui = await ur.json();
  const sid = await makeSessionId();
...
async function onLogout(request, env) {
  const sid = cookieSession(request);
...
async function onSession(request, env) {
  const s = await getSession(request, env);
  if (!s) return json({ authenticated: false });
  return json({ authenticated: true, user: s.user });
}

async function onChat(request, env) {
  ...
  try {
    ...
    let cd;
    try { const ex = await env.CHATS.get(key); cd = ex ? JSON.parse(ex) : { messages: [], created_at: Date.now(), first_question: question }; }
    ...
    return json({ chatId: cid, council, responses, synthesis });
  } catch (err) { return json({ error: err.message || 'Processing failed' }, 500); }
}

async function onHistory(request, env) {
  const s = await getSession(request, env);
  if (!s) return json({ error: 'Unauthorized' }, 401);
  const chatId = new URL(request.url).searchParams.get('chatId');
  if (chatId) {
    try {
      const raw = await env.CHATS.get('chat:' + s.user.id + ':' + chatId);
      if (!raw) return json({ messages: [] });
...
#lp{position:fixed;inset:0;z-index:100;display:flex;align-items:center;justify-content:center;background:var(--bg)}
#lp.h{display:none}
...
.bhf:hover{background:var(--gl);transform:translateY(-1px)}
.bhf svg{width:20px;height:20px}
.ln{margin-top:16px;font-size:12px;color:var(--tx3)}
#app{display:flex;height:100vh;position:relative;z-index:1}
#app.h{display:none}
#sb{width:260px;flex-shrink:0;background:var(--sur);border-right:1px solid var(--bor);display:flex;flex-direction:column;transition:width .25s ease,opacity .25s ease;overflow:hidden}
#sb.c{width:0;opacity:0}
...
.bnc:hover{background:rgba(201,164,74,.18)}
.bnc svg{width:14px;height:14px}
...
.ui{flex:1;overflow:hidden}
.un{font-size:13px;font-weight:500;color:var(--tx);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.us{font-size:11px;color:var(--tx3)}
.blo{background:none;border:none;cursor:pointer;color:var(--tx3);padding:4px;border-radius:4px;transition:color var(--t);display:flex;align-items:center}
.blo:hover{color:var(--err)}
.blo svg{width:16px;height:16px}
...
.bst{background:none;border:none;cursor:pointer;color:var(--tx2);padding:6px;border-radius:6px;display:flex;align-items:center;transition:color var(--t),background var(--t)}
.bst:hover{color:var(--tx);background:var(--sur2)}
.bst svg{width:18px;height:18px}
...
.eq{padding:8px 16px;background:var(--sur2);border:1px solid var(--bor2);border-radius:20px;font-size:13px;color:var(--tx2);cursor:pointer;transition:background var(--t),border-color var(--t),color var(--t)}
.eq:hover{background:rgba(201,164,74,.1);border-color:rgba(201,164,74,.3);color:var(--gold)}
...
.ch{padding:12px 18px;border-bottom:1px solid var(--bor);display:flex;align-items:center;gap:10px;background:rgba(201,164,74,.03)}
.cl{font-family:var(--fm);font-size:10px;color:var(--gd);letter-spacing:.1em;text-transform:uppercase}
.cc{font-size:11px;color:var(--tx3);margin-left:auto}
.ts{overflow-x:auto}
.ct{width:max-content;min-width:100%;border-collapse:collapse}
.pt{min-width:300px;max-width:340px;padding:18px 20px 14px;vertical-align:top;border-right:1px solid var(--bor);border-bottom:3px solid transparent;text-align:left}
.pt:last-child{border-right:none}
.ph{display:flex;align-items:flex-start;gap:12px;margin-bottom:8px}
.pa{width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:var(--fu);font-size:13px;font-weight:700;flex-shrink:0}
.pn{font-family:var(--fd);font-size:17px;font-weight:700;color:var(--tx);line-height:1.2}
.pti{font-size:12px;color:var(--tx2);margin-top:2px;font-weight:400;line-height:1.3}
...
.pb{margin-top:6px;font-size:11px;font-style:italic;color:var(--tx3);font-family:var(--fb);line-height:1.4}
.rt{padding:16px 20px;vertical-align:top;border-right:1px solid var(--bor);border-left:3px solid transparent}
...
.lt2{font-family:var(--fd);font-size:18px;font-weight:600;color:var(--tx2)}
.lps{display:flex;flex-direction:column;gap:10px;margin-top:16px}
.lp2{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:var(--r);background:var(--sur2);border:1px solid var(--bor);font-size:13px;color:var(--tx2);transition:all .3s ease}
.lp2.ac{border-color:rgba(201,164,74,.3);background:rgba(201,164,74,.05);color:var(--gold)}
.lp2.dn{color:var(--ok);border-color:rgba(58,170,114,.2);background:rgba(58,170,114,.05)}
.pd{width:8px;height:8px;border-radius:50%;background:var(--tx3);flex-shrink:0}
.lp2.ac .pd{background:var(--gold);animation:pulse 1.2s ease-in-out infinite}
.lp2.dn .pd{background:var(--ok)}
@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.7)}}
.sw{display:flex;flex-direction:column;gap:12px;margin-top:4px}
.sc2{background:var(--sur);border:1px solid var(--bor2);border-radius:var(--r2);overflow:hidden}
.sch2{padding:12px 18px;border-bottom:1px solid var(--bor);display:flex;align-items:center;gap:10px;cursor:pointer;user-select:none;transition:background var(--t)}
.sch2:hover{background:rgba(255,255,255,.02)}
.si{width:28px;height:28px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0}
.st{font-family:var(--fd);font-size:16px;font-weight:700;color:var(--tx)}
.ss{font-size:12px;color:var(--tx3);margin-left:auto}
.sv{margin-left:auto;color:var(--tx3);transition:transform .2s ease}
.sv.o{transform:rotate(180deg)}
.sb2{padding:18px 20px;display:none}
.sb2.o{display:block}
.ssum{font-family:var(--fb);font-size:16px;line-height:1.8;color:var(--tx)}
.ssum p+p{margin-top:12px}
.fs{margin-bottom:20px}
.fs:last-child{margin-bottom:0}
.fl{font-family:var(--fm);font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--gd);margin-bottom:10px;display:flex;align-items:center;gap:8px}
.fl::after{content:'';flex:1;height:1px;background:var(--bor)}
.fli{list-style:none;display:flex;flex-direction:column;gap:7px}
.fli li{display:flex;align-items:flex-start;gap:10px;font-family:var(--fb);font-size:15px;color:var(--tx);line-height:1.6}
.fli li::before{content:'→';color:var(--gd);flex-shrink:0;margin-top:1px;font-size:12px}
...
.oqw{font-family:var(--fb);font-size:14px;font-style:italic;color:var(--tx2);line-height:1.5}
.oqw::before{content:'Why unresolved: ';color:var(--tx3);font-style:normal;font-size:11px;text-transform:uppercase;letter-spacing:.06em}
#ia{padding:16px 24px 20px;border-top:1px solid var(--bor);background:rgba(7,9,26,.95);backdrop-filter:blur(10px);flex-shrink:0}
.iw{background:var(--sur);border:1px solid var(--bor2);border-radius:var(--r2);display:flex;flex-direction:column;transition:border-color .2s ease,box-shadow .2s ease}
.iw:focus-within{border-color:rgba(201,164,74,.4);box-shadow:0 0 0 3px rgba(201,164,74,.06)}
#qi{background:none;border:none;outline:none;padding:14px 16px 8px;font-family:var(--fb);font-size:16px;color:var(--tx);resize:none;max-height:160px;min-height:50px;line-height:1.6}
#qi::placeholder{color:var(--tx3)}
.it{display:flex;align-items:center;gap:8px;padding:8px 12px;border-top:1px solid var(--bor)}
.ih{font-size:11px;color:var(--tx3);font-family:var(--fm)}
.bs{margin-left:auto;display:flex;align-items:center;gap:7px;padding:8px 18px;background:var(--gold);border:none;border-radius:8px;font-family:var(--fu);font-size:13px;font-weight:600;color:#0b0e1a;cursor:pointer;transition:background var(--t),transform var(--t)}
.bs:hover:not(:disabled){background:var(--gl);transform:translateY(-1px)}
.bs:disabled{opacity:.4;cursor:not-allowed}
.bs svg{width:14px;height:14px}
...
<div id="lp" class="h">
  <div class="lc">
    <div class="ll">KWEN</div>
    <div class="lt">Critical Thinking Research Council</div>
    <ul class="lf">
      ...
      <li>Full chat history across sessions</li>
    </ul>
...
<div id="app" class="h">
  <nav id="sb">
    <div class="sbh">
      ...
      <button class="bnc" onclick="newChat()">
    ...
    <div class="sbf">
      ...
      <button class="blo" onclick="logout()">
  ...
  <div id="mn">
    <header class="mh">
      <button class="bst" onclick="toggleSB()">
    ...
    <div id="ia">
      <div class="iw">
        <textarea id="qi" rows="2" placeholder="Ask your council a question..." onkeydown="onKey(event)" oninput="resize(this)"></textarea>
        <div class="it">
          <span class="ih">Shift+Enter for new line</span>
          <button class="bs" id="sbtn" onclick="send()">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
            Convene Council
          </button>
        </div>
      </div>
      <p class="disc">For research &amp; education only — not professional medical, legal or financial advice.</p>
    </div>
  </div>
</div>
...
function stob(){document.getElementById('msgs').scrollTop=9999}
function toggleSB(){document.getElementById('sb').classList.toggle('c')}
...
function showApp(){
  document.getElementById('lp').classList.add('h');
  ...
  document.getElementById('unm').textContent=user.name||user.username;
}
function logout(){window.location.href='/api/auth/logout'}
function setQ(q){var i=document.getElementById('qi');i.value=q;i.focus();resize(i)}
function onKey(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}
function resize(el){el.style.height='auto';el.style.height=Math.min(el.scrollHeight,160)+'px'}

function newChat(){
  cid=null;hist=[];
  var m=document.getElementById('msgs');m.innerHTML='';
  ...
  m.appendChild(es);
  document.getElementById('htt').textContent='Council Chamber';
  document.getElementById('cbadge').style.display='none';
  document.querySelectorAll('.sci').forEach(function(el){el.classList.remove('a')});
}
...
async function send(){
  ...
  var ps=lw.querySelectorAll('.lp2');var pi=0;
  ...
  try{
    ...
    lw.remove();
    mb.appendChild(mkTable(d.council,d.responses));
    ...
    clearInterval(piv);
    lw.innerHTML='<p style="color:var(--err);padding:20px;text-align:center;font-family:var(--fb)">Error: '+e(err.message)+'. Please try again.</p>';
  }
...
function mkLoad(){
  ...
  d.innerHTML='<div class="lt2">Assembling your council</div><div class="lps"><div class="lp2"><div class="pd"></div>Analysing topic &amp; selecting specialists</div><div class="lp2"><div class="pd"></div>Locking epistemic identities</div><div class="lp2"><div class="pd"></div>Consulting each specialist independently</div><div class="lp2"><div class="pd"></div>Synthesising tensions &amp; open questions</div></div>';
  return d;
}
...
function mkTable(council,responses){
  ...
  hdr.innerHTML='<span class="cl">&#9670; Council Perspectives</span><span class="cc">'+council.length+' Specialists</span>';
  ...
  council.forEach(function(p,i){
    var c=CV[i%CV.length];var th=document.createElement('th');th.className='pt';th.style.borderBottomColor=c.bg;
    var ini=p.avatar_initials||(p.name||'?').split(' ').map(function(w){return w[0]}).join('').slice(0,2).toUpperCase();
    th.innerHTML='<div class="ph"><div class="pa" style="background:'+c.dk+';color:'+c.li+';border:1.5px solid '+c.bg+'">'+ini+'</div><div><div class="pn">'+e(p.name)+'</div><div class="pti">'+e(p.title)+'</div></div></div><span class="ptr" style="color:'+c.li+';border-color:'+c.bg+'40;background:'+c.dk+'">'+e(p.intellectual_tradition)+'</span><div class="pb">Bias: '+e(p.known_bias)+'</div>';
    hr.appendChild(th);
  });
  thead.appendChild(hr);tbl.appendChild(thead);
  var tbody=document.createElement('tbody');var rr=document.createElement('tr');
  responses.forEach(function(r,i){
    var c=CV[i%CV.length];var td=document.createElement('td');td.className='rt';td.style.borderLeftColor=c.bg;
    var txt=e(r).replace(/\n\n/g,'</p><p>').replace(/\n/g,'<br>');
    td.innerHTML='<div class="rtx"><p>'+txt+'</p></div>';
    rr.appendChild(td);
  });
  tbody.appendChild(rr);tbl.appendChild(tbody);scroll.appendChild(tbl);wrap.appendChild(scroll);
  return wrap;
}

function mkSynth(syn){
  var wrap=document.createElement('div');wrap.className='sw fi';
  wrap.appendChild(mkCard('&#9670;','rgba(201,164,74,.15)','var(--gold)','Council Summary','Key agreements, tensions & positions',function(b){
    var d=document.createElement('div');d.className='ssum';
    d.innerHTML=(syn.summary||'').split('\n\n').filter(Boolean).map(function(p){return '<p>'+e(p)+'</p>'}).join('');
    b.appendChild(d);
  }));
  var fw=syn.decision_framework||{};
  wrap.appendChild(mkCard('&#9671;','rgba(74,139,201,.12)','#4a8bc9','Decision Framework','Questions, evidence thresholds & red flags',function(b){
    [{l:'Critical Questions',items:fw.key_questions},{l:'Evidence That Would Shift Views',items:fw.evidence_that_would_change_views},{l:'Red Flags',items:fw.red_flags}].forEach(function(s){
      if(!s.items||!s.items.length)return;
      var sec=document.createElement('div');sec.className='fs';
      sec.innerHTML='<div class="fl">'+e(s.l)+'</div>';
      var ul=document.createElement('ul');ul.className='fli';
      s.items.forEach(function(it){var li=document.createElement('li');li.textContent=it;ul.appendChild(li)});
  ...
  wrap.appendChild(mkCard('?','rgba(122,74,201,.12)','#7a4ac9','Open Questions','What remains unresolved & why',function(b){
    ...
    (syn.open_questions||[]).forEach(function(oq,i){
      ...
      it.innerHTML='<div class="oqn">QUESTION '+(i+1)+'</div><div class="oqq">'+e(oq.question)+'</div><div class="oqw">'+e(oq.why_unresolved)+'</div>';
      d.appendChild(it);
    });b.appendChild(d);
  }));
  return wrap;
...
function mkCard(icon,ibg,ic,title,sub,fn){
  var card=document.createElement('div');card.className='sc2';
  var hdr=document.createElement('div');hdr.className='sch2';
  hdr.innerHTML='<div class="si" style="background:'+ibg+';color:'+ic+'">'+icon+'</div><span class="st">'+title+'</span><span class="ss">'+sub+'</span><svg class="sv o" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><polyline points="18 15 12 9 6 15"/></svg>';
  var body=document.createElement('div');body.className='sb2 o';fn(body);
  hdr.addEventListener('click',function(){body.classList.toggle('o');hdr.querySelector('.sv').classList.toggle('o')});
  card.appendChild(hdr);card.appendChild(body);return card;
}

async function loadHist(){
  try{
    var r=await fetch('/api/history',{credentials:'same-origin'});
    var d=await r.json();var list=document.getElementById('chl');
    if(d.chats&&d.chats.length){
      list.innerHTML=d.chats.map(function(c){return '<div class="sci'+(c.id===cid?' a':'')+'" onclick="loadChat(\''+c.id+'\')"><div class="sci-t">'+e(c.first_question||'Session')+'</div><div class="sci-d">'+new Date(c.created_at).toLocaleDateString()+'</div></div>'}).join('');
    }
  }catch(x){}
}
...
async function loadChat(chatId){
  try{
    var r=await fetch('/api/history?chatId='+chatId,{credentials:'same-origin'});
    var d=await r.json();
    if(d&&d.messages&&d.messages.length){
      ...
      d.messages.forEach(function(m){
        ...
        if(m.synthesis)mb.appendChild(mkSynth(m.synthesis));
        msgs.appendChild(mb);
      });
...
// ================================================================
// SERVICE WORKER ENTRY (Pages-compatible format)
// ================================================================
...
async function handleRequest(request, event) {
  var env = {
    ...
    CHATS: typeof CHATS !== 'undefined' ? CHATS : null,
    HF_CLIENT_ID: typeof HF_CLIENT_ID !== 'undefined' ? HF_CLIENT_ID : '',
    HF_CLIENT_SECRET: typeof HF_CLIENT_SECRET !== 'undefined' ? HF_CLIENT_SECRET : '',
    REDIRECT_URI: typeof REDIRECT_URI !== 'undefined' ? REDIRECT_URI : '',
    APP_URL: typeof APP_URL !== 'undefined' ? APP_URL : 'https://conspread.pages.dev',
    HF_MODEL: typeof HF_MODEL !== 'undefined' ? HF_MODEL : 'Qwen/Qwen2.5-72B-Instruct'
  };

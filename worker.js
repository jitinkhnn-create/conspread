// ================================================================
// KWEN COUNCIL — Cloudflare Worker
// Critical Thinking Research & Education Tool
// ================================================================

// ================================================================
// SAFE AI GUIDELINES — Injected into every AI call
// ================================================================
...
// ================================================================
// PERSONA COLORS — Each persona gets a consistent identity color
// ================================================================
const PERSONA_COLORS = [
  { bg: '#c9844a', light: '#e8a96a', text: '#0b0e1a' },
  { bg: '#4a8bc9', light: '#6aaae8', text: '#0b0e1a' },
  { bg: '#c94a72', light: '#e86a92', text: '#0b0e1a' },
  { bg: '#7a4ac9', light: '#9a6ae8', text: '#0b0e1a' },
  { bg: '#3aaa72', light: '#5aca92', text: '#0b0e1a' },
];
...
- id: short_snake_case (e.g. "empiricist_scientist")
- name: Full Name (realistic, diverse)
- title: Academic or Professional Title
- intellectual_tradition: Named school of thought (e.g. "Keynesian economics", "Phenomenological philosophy", "Evidence-based medicine")
...
- friction_with: Array of 2-3 specific types of reasoning or assumptions they ALWAYS push back against
- forbidden_rhetoric: Array of 3-4 phrases or rhetorical patterns they would NEVER use (this enforces voice distinctiveness)
- vocabulary_register: One of exactly: "highly_technical" | "accessible_academic" | "philosophical" | "empirical_quantitative" | "narrative_qualitative" | "policy_pragmatic"
- known_bias: Their acknowledged intellectual blind spot — be honest and specific
- signature_approach: How they structure arguments (e.g. "always historicizes before analyzing", "leads with mechanism then implication", "starts by identifying what the question assumes")
- avatar_initials: 2 capital letters from their name

Return ONLY a valid JSON array. No markdown fences, no explanation, no preamble. Just the JSON array.`;
}
...
HARD ANTI-HOMOGENIZATION RULES — Violating these defeats your purpose:
...
3. You write ONLY in ${persona.vocabulary_register} register — this makes you identifiable
...
7. If you agree with something mainstream, you explain WHY through YOUR unique framework, not in generic terms

Other council members: ${others}. You are aware of their existence but you respond independently before hearing them.

CONTEXT: This is a critical thinking research and education tool. Your role is genuine intellectual engagement, not diplomatic balance.
...
You are a synthesis engine for a multi-perspective critical thinking council. Your task: synthesize the council's responses WITHOUT papering over genuine disagreements.
...
COMPONENT 1 — SUMMARY:
- Identify the 2-3 most important points of GENUINE DISAGREEMENT (go beneath surface differences to root epistemic/value conflicts)
- Identify any unexpected convergences across different traditions
- Do NOT force consensus where none exists — preserve the productive tension
- Name which council members hold which positions

COMPONENT 2 — DECISION FRAMEWORK:
- key_questions: 3-5 questions a critical thinker should ask (derived from the council's tensions, not generic)
- evidence_that_would_change_views: For each perspective, what evidence or argument would shift it (shows intellectual honesty)
- red_flags: 3-4 warning signs or traps that the council collectively identified

COMPONENT 3 — OPEN QUESTIONS:
3-5 questions that remain GENUINELY unresolved after the council, each with a specific reason WHY it's hard to resolve (methodological, empirical, definitional, or political).
...
// ================================================================
// FALLBACK COUNCIL — Used if AI council generation fails
// ================================================================
function getDefaultCouncil(topic) {
  return [
    {
      id: "empirical_scientist",
      name: "Dr. Morgan Wells",
      title: "Empirical Research Scientist",
      intellectual_tradition: "Scientific empiricism",
      epistemology: "Knowledge is validated through reproducible evidence and falsifiable hypotheses, not through intuition or authority.",
      core_commitment: "Claims require evidence proportional to their extraordinary nature; absence of evidence is meaningful data.",
      ...
      known_bias: "Systematically undervalues qualitative, experiential, and indigenous knowledge systems.",
      signature_approach: "Leads with what the evidence shows, then explicitly states what the evidence cannot tell us.",
      avatar_initials: "MW"
    },
    {
      id: "critical_philosopher",
      name: "Prof. Aria Chen",
      title: "Philosopher of Knowledge & Critical Theory",
      intellectual_tradition: "Critical rationalism with post-structural influences",
      epistemology: "Understanding requires examining the power structures and assumptions embedded in the question itself before attempting to answer it.",
      core_commitment: "Every framework for understanding contains hidden normative and political commitments that must be surfaced.",
      friction_with: ["naive empiricism that ignores social construction", "instrumental reasoning", "false value-neutrality"],
      ...
      known_bias: "Can over-complicate questions that have clear practical answers, paralysing action through infinite regress.",
      signature_approach: "Always historicises and deconstructs the question before answering it.",
      avatar_initials: "AC"
    },
    {
      id: "policy_pragmatist",
      name: "Dr. James Okafor",
      title: "Applied Policy Researcher & Institutional Economist",
      intellectual_tradition: "Pragmatic institutionalism",
      epistemology: "Truth is what produces workable outcomes within real constraints of institutions, incentives, and power.",
      core_commitment: "Abstract principles must survive contact with institutional realities and political economy to be meaningful.",
      friction_with: ["purely theoretical approaches disconnected from implementation", "solutions that ignore incentive structures", "ahistorical analysis"],
      ...
      known_bias: "Can be too accommodating of existing power structures, treating unjust institutions as inevitable constraints.",
      signature_approach: "Leads with the institutional context and power dynamics, then evaluates what is actually achievable.",
      avatar_initials: "JO"
    }
  ];
}
...
:root {
  ...
  --c0: #c9844a; --c0l: #e8a96a;
  --c1: #4a8bc9; --c1l: #6aaae8;
  --c2: #c94a72; --c2l: #e86a92;
  --c3: #7a4ac9; --c3l: #9a6ae8;
  --c4: #3aaa72; --c4l: #5aca92;
}
...
body::before {
  ...
  background: 
    radial-gradient(ellipse 60% 50% at 20% 20%, rgba(201,164,74,0.04) 0%, transparent 60%),
...
.login-card {
  ...
  position: relative;
  box-shadow: 0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(201,164,74,0.1);
}
.login-card::before {
  content: '';
...
.btn-hf:hover {
  ...
  box-shadow: 0 8px 24px rgba(201,164,74,0.3);
}
...
.loading-phase.done {
  color: var(--success);
  border-color: rgba(58,170,114,0.2);
  background: rgba(58,170,114,0.05);
}
.phase-dot {
...
.framework-list li::before {
  content: '→';
  color: var(--gold-dim); flex-shrink: 0; margin-top: 1px;
  font-size: 12px;
}
...
.oq-why {
  ...
  font-style: italic; color: var(--text2); line-height: 1.5;
}
...
@media (max-width: 700px) {
  #sidebar { position: absolute; top: 0; bottom: 0; left: 0; z-index: 50; }
  .persona-th { min-width: 260px; }
}

.disclaimer {
  font-size: 11px; color: var(--text3); text-align: center;
  padding: 8px 0 0; font-family: var(--font-body); font-style: italic;
}
</style>
</head>
<body>
...
async function sendMessage() {
  ...
  const phaseInterval = setInterval(() => {
    if (phaseIdx > 0) phases[phaseIdx-1].classList.remove('active'), phases[phaseIdx-1].classList.add('done');
    if (phaseIdx < phases.length) phases[phaseIdx].classList.add('active');
    phaseIdx++;
  ...
  try {
    ...
    const tableEl = renderCouncilTable(data.council, data.responses);
    msgBlock.appendChild(tableEl);

    // Render synthesis
    if (data.synthesis) {
      const synthEl = renderSynthesis(data.synthesis);
      msgBlock.appendChild(synthEl);
    }
...
function createEmptyState() {
  ...
  d.innerHTML = document.querySelector('.empty-state') ? document.querySelector('.empty-state').innerHTML : '';
  return d;
}

function createLoadingUI() {
  const d = document.createElement('div');
  d.className = 'loading-wrap fade-in';
...
const COLOR_VARS = [
  { bg: '#c9844a', light: '#e8a96a', dark: '#3d2010' },
  { bg: '#4a8bc9', light: '#6aaae8', dark: '#10203d' },
  { bg: '#c94a72', light: '#e86a92', dark: '#3d1020' },
  { bg: '#7a4ac9', light: '#9a6ae8', dark: '#1e1040' },
  { bg: '#3aaa72', light: '#5aca92', dark: '#0d3020' },
];

function renderCouncilTable(council, responses) {
  const wrap = document.createElement('div');
  wrap.className = 'council-wrap fade-in';
  
  const header = document.createElement('div');
  header.className = 'council-header';
  header.innerHTML = '<span class="council-label">&#9670; Council Perspectives</span><span class="council-count">' + council.length + ' Specialists</span>';
  ...
  council.forEach((p, i) => {
    const c = COLOR_VARS[i % COLOR_VARS.length];
    const th = document.createElement('th');
    th.className = 'persona-th';
    th.style.borderBottomColor = c.bg;
    th.innerHTML =
      '<div class="persona-header">' +
        '<div class="persona-avatar" style="background:' + c.dark + ';color:' + c.light + ';border:1.5px solid ' + c.bg + '">' + (p.avatar_initials || p.name.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase()) + '</div>' +
        '<div><div class="persona-name">' + escHtml(p.name) + '</div><div class="persona-title">' + escHtml(p.title) + '</div></div>' +
      '</div>' +
      '<span class="persona-tradition" style="color:' + c.light + ';border-color:' + c.bg + '40;background:' + c.dark + '">' + escHtml(p.intellectual_tradition) + '</span>' +
      '<div class="persona-bias">Bias: ' + escHtml(p.known_bias) + '</div>';
    hrow.appendChild(th);
  });
  ...
  responses.forEach((r, i) => {
    ...
    td.style.borderLeftColor = c.bg;
    td.innerHTML = '<div class="response-text">' + escHtml(r).replace(/\n\n/g,'</p><p>').replace(/\n/g,'<br>').replace(/^/, '<p>').replace(/$/, '</p>') + '</div>';
    rrow.appendChild(td);
  });
...
function renderSynthesis(synthesis) {
  const wrap = document.createElement('div');
  wrap.className = 'synthesis-wrap fade-in';

  // Summary
  wrap.appendChild(makeSynthCard(
    '&#9670;',
    'rgba(201,164,74,0.15)',
    'var(--gold)',
    'Council Summary',
    'Key agreements, tensions &amp; positions',
    function(body) {
      const div = document.createElement('div');
      div.className = 'synth-summary';
      const paras = (synthesis.summary || '').split('\n\n').filter(Boolean);
      div.innerHTML = paras.map(p => '<p>' + escHtml(p) + '</p>').join('');
  ...
  wrap.appendChild(makeSynthCard(
    '&#9671;',
    'rgba(74,139,201,0.12)',
    '#4a8bc9',
    'Decision Framework',
    'Questions, evidence thresholds &amp; red flags',
    function(body) {
      const sections = [
        { label: 'Critical Questions to Ask', items: fw.key_questions },
        { label: 'Evidence That Would Shift Each View', items: fw.evidence_that_would_change_views },
        { label: 'Red Flags & Warning Signs', items: fw.red_flags },
      ];
      sections.forEach(s => {
        if (!s.items || !s.items.length) return;
        const sec = document.createElement('div');
        sec.className = 'framework-section';
        sec.innerHTML = '<div class="framework-label">' + escHtml(s.label) + '</div>';
  ...
  // Open Questions
  const oqs = synthesis.open_questions || [];
  wrap.appendChild(makeSynthCard(
    '?',
    'rgba(122,74,201,0.12)',
    '#7a4ac9',
    'Open Questions',
    'What remains genuinely unresolved &amp; why',
    function(body) {
      const div = document.createElement('div');
      ...
      oqs.forEach((oq, i) => {
        ...
        item.innerHTML =
          '<div class="oq-num">QUESTION ' + (i+1) + '</div>' +
          '<div class="oq-question">' + escHtml(oq.question) + '</div>' +
          '<div class="oq-why">' + escHtml(oq.why_unresolved) + '</div>';
...
function makeSynthCard(iconChar, iconBg, iconColor, title, sub, populateFn) {
  const card = document.createElement('div');
  card.className = 'synthesis-card';

  const header = document.createElement('div');
  header.className = 'synthesis-card-header';
  header.innerHTML =
    '<div class="synth-icon" style="background:' + iconBg + ';color:' + iconColor + '">' + iconChar + '</div>' +
    '<span class="synth-title">' + title + '</span>' +
    '<span class="synth-sub">' + sub + '</span>' +
    '<svg class="synth-chevron open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><polyline points="18 15 12 9 6 15"/></svg>';

  const body = document.createElement('div');
  body.className = 'synthesis-body open';
  populateFn(body);

  header.addEventListener('click', () => {
    body.classList.toggle('open');
    header.querySelector('.synth-chevron').classList.toggle('open');
  });
    ...
    if (data.chats && data.chats.length > 0) {
      list.innerHTML = data.chats.map(c =>
        '<div class="chat-history-item' + (c.id === currentChatId ? ' active' : '') + '" onclick="loadChat(\'' + c.id + '\')">' +
          '<div class="chi-title">' + escHtml(c.first_question || 'Session') + '</div>' +
          '<div class="chi-date">' + new Date(c.created_at).toLocaleDateString() + '</div>' +
        '</div>'
      ).join('');
    }
  } catch(e) { /* Silent fail */ }
...
async function loadChat(chatId) {
  try {
    ...
    if (data && data.messages && data.messages.length > 0) {
      ...
      chatHistory = data.messages.map(m => ({ question: m.question, responses: m.responses }));
      ...
      data.messages.forEach(m => {
        ...
        msgBlock.appendChild(userMsg);
        if (m.council && m.responses) msgBlock.appendChild(renderCouncilTable(m.council, m.responses));
        if (m.synthesis) msgBlock.appendChild(renderSynthesis(m.synthesis));
        msgs.appendChild(msgBlock);
      });

      const lastQ = data.messages[data.messages.length-1].question;
      document.getElementById('header-title').textContent = lastQ.slice(0,50) + (lastQ.length>50?'...':'');
      document.getElementById('council-badge').style.display = '';
      scrollToBottom();

      document.querySelectorAll('.chat-history-item').forEach(el => el.classList.remove('active'));
      document.querySelectorAll('.chat-history-item').forEach(el => {
        if (el.getAttribute('onclick') && el.getAttribute('onclick').includes(chatId)) el.classList.add('active');
      });
    }
...
// ================================================================
async function callQwen(messages, hfToken, env, opts = {}) {
  ...
  if (!resp.ok) {
    ...
    throw new Error('HuggingFace API error ' + resp.status + ': ' + errText.slice(0, 200));
  }
...
// COUNCIL LOGIC
// ================================================================
async function generateCouncil(topic, hfToken, env) {
  const text = await callQwen(
    [{ role: 'user', content: getCouncilGenerationPrompt(topic) }],
    hfToken, env,
    { max_tokens: 2500, temperature: 0.88 }
  );

  try {
    const clean = text.replace(/```json\s?/g, '').replace(/```\s?/g, '').trim();
    const council = JSON.parse(clean);
    if (!Array.isArray(council) || council.length < 2) throw new Error('Invalid council format');
    return council.slice(0, 5); // max 5 personas
  } catch (_e) {
    return getDefaultCouncil(topic);
  }
}

async function getPersonaResponse(persona, allPersonas, question, history, hfToken, env) {
  const personaIdx = allPersonas.findIndex(p => p.id === persona.id);

  // Build a minimal relevant history for this persona
  const historyMessages = history.slice(-3).flatMap(h => {
    const pastResp = h.responses && h.responses[personaIdx] ? h.responses[personaIdx] : '';
    if (!pastResp) return [{ role: 'user', content: h.question }];
    return [
      { role: 'user', content: h.question },
      { role: 'assistant', content: pastResp },
    ];
  });
  ...
  return callQwen(messages, hfToken, env, { max_tokens: 450, temperature: 0.74 });
}

async function generateSynthesis(question, council, responses, hfToken, env) {
  const text = await callQwen(
    [{ role: 'user', content: getSynthesisPrompt(question, council, responses) }],
    hfToken, env,
    { max_tokens: 1800, temperature: 0.62 }
  );
...
async function handleCallback(request, env) {
  ...
  if (errParam || !code) {
    return Response.redirect(env.APP_URL || 'https://' + new URL(request.url).hostname + '/?auth_error=1', 302);
  }
  ...
  const tokenResp = await fetch('https://huggingface.co/oauth/token', {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: env.REDIRECT_URI,
      client_id: env.HF_CLIENT_ID,
      client_secret: env.HF_CLIENT_SECRET,
    }),
  });
  ...
  if (!tokenData.access_token) {
    return Response.redirect((env.APP_URL || '/') + '?auth_error=no_token', 302);
  }
  ...
  await env.SESSIONS.put(sessionId, JSON.stringify(sessionData), { expirationTtl: 86400 });

  const appUrl = env.APP_URL || 'https://' + new URL(request.url).hostname;

  return new Response(null, {
    status: 302,
    headers: {
      'Location': appUrl,
      // HTTPS-only, HttpOnly, SameSite=Strict — maximum security
...
async function handleLogout(request, env) {
  const sid = getSessionIdFromCookie(request);
  if (sid) await env.SESSIONS.delete(sid).catch(() => {});
...
async function handleChat(request, env) {
  ...
  catch (_e) { return jsonResp({ error: 'Invalid JSON' }, 400); }

  const { question, chatId, history = [] } = body;

  if (!question || typeof question !== 'string' || question.trim().length === 0) {
    return jsonResp({ error: 'Question is required' }, 400);
  }
  if (question.length > 2000) {
    return jsonResp({ error: 'Question too long (max 2000 characters)' }, 400);
  }

  try {
    // Step 1: Dynamically assemble council for this topic
    const council = await generateCouncil(question.trim(), session.hf_token, env);

    // Step 2: All personas respond in PARALLEL (prevents cross-contamination)
    const responses = await Promise.all(
      council.map(persona => getPersonaResponse(persona, council, question, history, session.hf_token, env))
    );

    // Step 3: Synthesis (summary + framework + open questions)
    const synthesis = await generateSynthesis(question, council, responses, session.hf_token, env);
    ...
    const existing = await env.CHATS.get(chatKey);
    const chatData = existing ? JSON.parse(existing) : { messages: [], created_at: Date.now(), first_question: question };
    ...
    await env.CHATS.put(chatKey, JSON.stringify(chatData), { expirationTtl: 604800 }); // 7 days
...
async function handleHistory(request, env) {
  ...
  if (chatId) {
    const chatKey = 'chat:' + session.user.id + ':' + chatId;
    const raw = await env.CHATS.get(chatKey);
    if (!raw) return jsonResp({ messages: [] });
    return new Response(raw, { headers: { 'Content-Type': 'application/json' } });
  }

  // List chats — iterate known keys (limited without KV list, simplified here)
  // For full listing, D1 is recommended. This returns empty list for now.
